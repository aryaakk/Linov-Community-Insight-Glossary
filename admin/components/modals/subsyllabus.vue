<template>
    <div>
        <modal-basic id="subsyllabus" title="Add Syllabus Detail">
    ...
    export default {
        ...
        methods:{
            async submit(){
                let data, status
                if(this.form?.id){
                     await this.$updateSyllabus({details: this.form}, this.form.id).then(response=>{
                        data = response.data
                        status = response.status
                     })
                }else{
                     await this.$postSyllabus({details: this.form}).then(response=>{
                        data = response.data
                        status = response.status
                     })
                }
