<template>
    <div>
        <div class="card">
                ...
                @onChange="loadData"
                @onSearch="Search"
                @onCellClick="(data)=>{ $router.push(`/skills/${data.id}`) }" 
                @onCheckbox="(select)=>{selected=select}">
                    ...
                    <template v-slot:head-right>
                        <nuxt-link to="/skills/create">
                        <button class="btn btn-info px-2"><i class='bx bx-plus'></i> Add New Skills</button>
                        </nuxt-link>
    ...
    export default {
      ...
      methods: {
        async loadData(params){
            this.params = params
            const {data} = await this.$getSkills({params: params})
            this.datas = data
        },
        async Delete(){
            const {status} = await this.$deleteSkills(this.selected)
            if(status==204){
                this.selected=[]
