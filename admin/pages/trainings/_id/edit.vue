<template>
    <div>
        ...
        <div class="card mb-4">
            ...
            <h5 class="card-header">Create Training</h5>
            <div class="card-body">
                <div class="mb-3">
                ...
                <div class="mb-3 table-basic table-responsive text-nowrap">
                    <table class="table">
                        ...
                        <tbody>
                            <tr v-for="(schedule, idx) in form.schedules" :key="idx">
                                <td>{{ idx+1 }}</td>
                                <td>{{ schedule.date}}</td>
            ...
            <div class="card-footer d-flex justify-content-end">
                <nuxt-link to="/trainings" class="btn btn-danger mx-1"><i class='bx bx-x'></i>Batal</nuxt-link>
    ...
    export default {
      ...
      async mounted(){
        ...
        await this.$getOneTrainings(this.id).then(response=>{
            this.form = {...this.form, ...response.data,...{
                ...
                banner_slide: null, 
                banner_card: null, 
                trainer_id: response.data.trainers, 
                class: Object.assign({},response.data.classes[0])
            }}
            delete this.form.classes
        })
        await this.$getSchedule(this.id).then(response=>{
            this.form.schedules = response.data
        })
      },
      beforeDestroy() {
      ...
      methods: {
            async submit(){
                ...
                await this.$updateTrainEvents(form, this.id).then(response=>{
                    if(response.status==200){
                        this.$toast.success("Event berhasil diupdate!");
