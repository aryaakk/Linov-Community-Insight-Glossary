<template>
    <div>
        <div class="card">
                ...
                @onChange="loadData"
                @onSearch="Search"
                @onCellClick="(data)=>{ $router.push(`/trainings/${data.id}`) }" 
                @onCheckbox="(select)=>{selected=select}">
                    ...
                    <template v-slot:option="{ data, item }">
                        <nuxt-link :to="`/trainings/${data.id}`"><i class='bx bx-show'></i> Detail</nuxt-link><br>
                        <nuxt-link :to="`/trainings/${data.id}/edit`"><i class='bx bx-edit-alt'></i> Edit</nuxt-link><br>
                        <nuxt-link :to="`/trainings/${data.id}/syllabus`"><i class='bx bx-book-open'></i> Syllabus</nuxt-link>
                    </template>
                </basic-table>
         </div>
    </div>
    ...
    export default {
      ...
      methods: {
        async loadData(params){
            this.params = params
            const {data} = await this.$getTrainings({params: params})
            this.datas = data
        },
        async Delete(){
