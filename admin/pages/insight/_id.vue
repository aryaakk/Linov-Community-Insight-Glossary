<template>
    <div>
        <div class="card mb-4">
            ...
            <h5 class="card-header">Edit Insight</h5>
                ...
                <div class="col-3">
                    ...
                    <div class="py-3">
                        <div class="form-check form-check-inline">
                            <input :checked="form.open_discuss=='1'" class="form-check-input" type="checkbox" 
                            @input="form.open_discuss = $event.target.checked" id="comment" value="1">
                            <label class="form-check-label" for="comment">Open Discuss</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input :checked="form.is_ads=='1'" 
                            @input="form.is_ads = $event.target.checked"
                            class="form-check-input" type="checkbox" id="ads" value="1">
                            <label class="form-check-label" for="ads">Is Ads</label>
                        </div>
                    </div>
                    <hr>
                    <div class="py-3 d-grid gap-2">
                        <nuxt-link to="/insight" class="btn btn-danger"><i class='bx bx-x'></i>Batal</nuxt-link>
                        <button type="button" class="btn btn-secondary" @click="goPreview"><i class='bx bx-show' ></i> Preview</button>
                        <button class="btn btn-primary"><i class='bx bx-check' ></i> Update</button>
                    </div>
                </div>
            </div>
            </form>
...
import { Editor } from '@toast-ui/vue-editor'
    ...
    export default {
      ...
      async mounted(){
        ...
        await this.$getOneInsight(this.id).then(response=>{
            const data = response.data.data
            this.form = {...this.form, ...data,...{banner: null, published_date: this.$dayjs(data.published_date).format('YYYY-MM-DDTHH:mm:ss')}}
        })
      },
      methods: {
            async goPreview(){
            ...
            async submit(){
                ...
                await this.$updateInsight(form, this.id).then(response=>{
                    if(response.status==200){
                        this.$toast.success("Data insight berhasil diupdate!");
