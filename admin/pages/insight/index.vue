<template>
    <div>
        <div class="card">
                ...
                @onChange="loadData"
                @onSearch="Search"
                @onCellClick="(data)=>{ $router.push(`/insight/${data.id}`) }" 
                @onCheckbox="(select)=>{selected=select}">
                    ...
                    <template v-slot:head-right>
                        ...
                        <button class="btn btn-info px-2"><i class='bx bx-plus'></i> Add New Insight</button>
                        </nuxt-link>
                    ...
                    <template v-slot:title="{ data, item }">
                        <div style="min-width: 280px;white-space: normal !important;">{{ item }}</div>
                    </template>
                    <template v-slot:category="{ data, item }">
                        <div style="white-space: normal !important;">{{ item ? item.toString():'' }}</div>
                    </template>
                    <template v-slot:trx_code="{ data, item }">
                        <span class="badge me-1" :class="item=='ART-PRE' ? 'bg-label-info': 'bg-label-success'">
                            {{ item=='ART-PRE' ? 'Limited' : 'Public' }}</span>
                    </template>
                    <template v-slot:status="{ data, item }">
                        <span class="badge me-1" :class="item=='published' ? 'bg-label-success': 'bg-label-warning'">{{ item }}</span>
                    </template>
                </basic-table>
         </div>
    </div>
