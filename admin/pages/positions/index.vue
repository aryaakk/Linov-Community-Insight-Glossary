<template>
    <div>
        <div class="card">
                ...
                @onChange="loadData"
                @onSearch="Search"
                @onCellClick="(data)=>{ $router.push(`/positions/${data.id}`) }" 
                @onCheckbox="(select)=>{selected=select}">
                    ...
                    <template v-slot:head-right>
                        <nuxt-link to="/positions/create">
                        <button class="btn btn-info px-2"><i class='bx bx-plus'></i> Add New Posisi</button>
                        </nuxt-link>
    ...
    export default {
      ...
      methods: {
        async loadData(params){
            this.params = params
            const {data} = await this.$getPositions({params: params})
            this.datas = data
        },
        async Delete(){
            const {status} = await this.$deletePositions(this.selected)
            if(status==204){
                this.selected=[]
