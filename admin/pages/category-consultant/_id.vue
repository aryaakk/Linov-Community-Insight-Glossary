    ...
    <div>
        <div class="card mb-4">
            ...
            <div class="card-footer d-flex justify-content-end">
                <nuxt-link to="/category-consultant" class="btn btn-danger mx-1"><i class='bx bx-x'></i>Batal</nuxt-link>
...
<script>
    export default {
      ...
      async mounted(){
        await this.$getOneCatConsultant(this.id).then(response=>{
      ...
      methods: {
            async submit(){
                ...
                await this.$updateCatConsultant(this.form, this.id).then(response=>{
                    if(response.status==200){
                        this.$toast.success("Master data berhasil diupdate!");
                        this.$router.push(`/category-consultant`)
                    }
                }).catch(e=>{
