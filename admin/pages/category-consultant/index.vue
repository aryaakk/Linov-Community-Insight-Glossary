<template>
    <div>
        <div class="card">
                ...
                @onChange="loadData"
                @onSearch="Search"
                @onCellClick="(data)=>{ $router.push(`/category-consultant/${data.id}`) }" 
                @onCheckbox="(select)=>{selected=select}">
                    ...
                    <template v-slot:head-right>
                        <nuxt-link to="/category-consultant/create">
                        <button class="btn btn-info px-2"><i class='bx bx-plus'></i> Add New Category</button>
                        </nuxt-link>
...
<script>
    ...
    export default {
      head: {
        title: "Master Category Consultant",
      },
      ...
      methods: {
        async loadData(params){
            this.params = params
            const {data} = await this.$getCatConsultant({params: params})
            this.datas = data
        },
        async Delete(){
            const {status} = await this.$deleteCatConsultant(this.selected)
            if(status==204){
                this.selected=[]
