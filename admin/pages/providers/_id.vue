...
<script>
    export default {
      ...
      async mounted(){
        const {data} = await this.$getOneProviders(this.id)

        await this.$getState({params: {all: true}}).then(response=>{
            this.locations = response.data
        })

        this.form = {...this.form, ...data, ...{logo:null, logo_2:null, is_active: data.is_active=='1'}}
      },
      methods: {
            async submit(){
                ...
                await this.$updateProviders(form, this.id).then(response=>{
