            ...
            <div class="card-body row">
                ...
                <div class="py-2 col-md-6">
                    ...
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input" v-model="form.is_active" type="checkbox" id="is_active">
                ...
                <div class="mb-4">
                    ...
                    <ul class="list-group">
                        <li class="list-group-item d-flex justify-content-between" v-for="(edu, idx) in form.educations" :key="idx">
                            <div>
                                <h5>{{ edu.university_name || edu.other_university }}</h5>
                                <span>{{  edu.major_name || edu.other_major }} - {{  edu.degree_name || edu.other_title }}</span><br>
                ...
                <div class="mb-4">
                    ...
                    <hr>
                    <div>
                        <span class="badge bg-secondary px-2" v-for="(skill, idx) in form.skills">
...
import Certification from '~/components/modals/certification.vue'
    export default {
      ...
      async mounted(){
        this.$store.commit('SET_BREADCRUMB', `consultant/${this.id}`) 

        await this.$getOneConsultant(this.id).then((response)=>{
      ...
      methods: {
            async submit(){
                ...
                await this.$updateConsultant(form, this.id).then(response=>{
                    if(response.status==200){
                        this.$toast.success("Consultant berhasil diupdate!");
            ...
            addEducation(data){
                data.university_name = data.university_id.name
                data.university_id = data.university_id.id

                data.degree_name = data.title_degree_id.name
                data.title_degree_id = data.title_degree_id.id

                data.major_name = data.major_id.name
                data.major_id = data.major_id.id
