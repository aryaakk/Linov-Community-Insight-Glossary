                ...
                <div class="col-md-9">
                    ...
                    <div class="panel-subhead col-md-12 title-form">
                        ...
                        <div class="title-subhead">
                            <label>Edit Threads & Discuss</label>
                        </div>
                    </div>
                    ...
                    <div class="panel-subhead col-md-12">
                        <div class="main-thread page-form">
                            ...
                            <div class="row-main-thread">
                                <form enctype="multipart/form-data" class="form-thread" @submit.prevent="updateThread">
                                    ...
                                    <div class="form-group" v-if="polling.description" v-tooltip="`Sorry, 'Polling' cannot be edited. Please create a new thread for changes.`">
                                    ...
                                    <div class="row py-4">
                                      ...
                                      <div class="col-md-4 py-4 col-sm-12">
                                        <button :disabled="form.processing" type="submit" class="btn btn-primary btn-block"><i class="fas fa-file-alt"></i> Update Thread</button>
                                      </div>
                                    </div>
                                </form>
                            </div>
...
export default {
 ...
 async mounted() {
    ...
    await this.$getOneThread(this.id).then(data => {
        const thread = data.data
        const ids  = thread.types.map(t=>{return t.type_question_id})
        const question_id = this.categories.filter(cat=>ids.includes(cat.id))
        this.form = {...this.form, ...{title: thread.title, description:thread.description, question_id:question_id}}

        if(thread.pollings){
            this.polling.description = thread.pollings.description
            this.polling.options = thread.pollings.details.map(detail=>{return detail.poll_name})
            this.polling.duration= thread.pollings.duration_id
        }
    })
  },
  methods: {
    ...
    async updateThread(){
        ...
        await this.$updateThread(form, this.id).then(response=>{
            if(response.status==201){
                this.$toast.success("Thread berhasil diupdate!");
