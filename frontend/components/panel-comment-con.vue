    ...
    <div class="panel-subhead">
        ...
        <div class="main-thread">
            <div class="d-flex head-comment">
                ...
                <label class="lblAvaThread mt-2">{{ comment.author?.name }} 
                    <img src="/img/icon/ic_verified.svg" v-if="comment.author.badge=='1'" style="height: 20px !important; margin-top: 4px;">
                </label>
            </div>
            ...
            <div class="hiden-content-thread" v-if="!$auth.loggedIn" style="bottom: 26%;">
                <div class="panel-ads-comment">
                    <NuxtLink to="/consultation/create">
                    <div class="row">
                    <div class="col-xs-2">
                    <img src="/img/thread/consul-verified.png" class="mr-2">
                    </div>
                    <div class="col-xs-10">
                        <h4>Ingin Tahu Lebih Lanjut?</h4>
                        <span>Yuk, Konsultasi Ke HR Berpengalaman Sekarang Juga!</span>
                    </div>
                    </div>
                    </NuxtLink>
                </div>
            ...
            <div class="d-flex justify-content-between pt-2 mt-4" style=" border-top: 1px solid #E6E6E6;">
                ...
                <div class="pt-2 d-flex justify-content-right">
                    <span class="span-comment span-footer-thread cursor" @click="toggleReply" v-if="comment.hasOwnProperty('subcomment') && validUser">
            ...
            <div class="pt-4" v-if="$auth.loggedIn && validUser" v-show="comment.show_reply">
                <div class="d-flex head-comment">
            ...
            <div class="pt-3" v-if="haveSubcomment">
                <div v-show="comment.show_sub" class="mb-4" style="border-left: 1px solid #E6E6E6;">
                    <panel-comment-con @onDelete="(id)=>{ $emit('onDelete', id) }" :comment="comm" v-for="(comm, idx) in comment.subcomment" :key="idx"></panel-comment-con>
                </div>
...
export default {
  ...
  computed:{
    ...
    validUser(){
        return this.$auth.loggedIn && this.$auth?.user?.id==this.comment.user_id || this.$auth?.user?.is_consultant=='1'
    }
  },
  methods: {
    async postComment(){
        ...
        form.append('thread_id', this.comment.consul_thread_id)
        
        try {
            const { data } =  await this.$postConsultComment(form, this.comment.id)
            this.form.comment =''
            this.form.photos =[]
            this.form.upfiles =[]
    ...
    async handleAction(status){
        if(status){
            ...
            const { data } = await this.$deleteConsultComment({}, this.comment.id);
